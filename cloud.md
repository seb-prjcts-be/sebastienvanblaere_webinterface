...
- 2026-06-14: **Constellation liep achter op `index.php`** — bijgewerkt: `waves`-item weg (index commentte het uit), `p5waves` herlabeld naar "p5waves.org (library + service)", Resume-sectie + `curriculum vitae` toegevoegd (cv leeft op de hub-host, niet op prjcts.be → eigen `$cv_url` i.p.v. `$svc_url`), Buy-me-a-coffee-icoon (`bi-cup-hot-fill` → buymeacoffee.com/prjcts) naast LinkedIn in zowel `index.php` als constellation-socials. Les: bij elke `_shared/`-edit de 3 satelliet-kopieën mee-propageren; **prjcts.be-kopie is CRLF**, de rest LF → behoud line-endings (`sed 's/$/\r/'` voor prjcts) om git-diffs schoon te houden. p5waves.org rendert een eigen custom homepage zónder shared header → constellation verschijnt daar niet, maar `_shared`-kopie toch meegesynct.
